...
export default class HelloRouter implements CustomRouteOptions<HelloRequest> {
  ...
  handler: CustomRequestHandler<HelloRequest> = async function getHelloWorld (request: CustomRequest<HelloRequest>, reply: FastifyReply): Promise<void> {
    return await reply.code(200).send(request.query.test ? 'Hello w/ params' : 'Hello w/o params')
  }
}
